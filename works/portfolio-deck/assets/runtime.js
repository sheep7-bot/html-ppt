...
(function(){
  ...
  function defaultLayout() {
    ...
    return {
      'card-cur':   { x: 16,        y: 16,             w: Math.round(w*0.55) - 24, h: Math.round(h*0.70) - 16 },   // 左侧预览 70%
      'card-nxt':   { x: Math.round(w*0.55) + 8, y: 16, w: w - Math.round(w*0.55) - 24, h: Math.round(h*0.30) - 16 }, // 右上预览 30%
      'card-notes': { x: Math.round(w*0.55) + 8, y: Math.round(h*0.30) + 8, w: w - Math.round(w*0.55) - 24, h: h - Math.round(h*0.30) - 16 }, // 右侧整个下半：演讲稿
      'card-timer': { x: 16,        y: Math.round(h*0.70) + 8, w: Math.round(w*0.55) - 24, h: h - Math.round(h*0.70) - 16 } // 左下倒计时缩小
    };
  }
